...
contract OptionContract {
    ...
    event OptionExercised(address indexed buyer, address indexed seller, uint index);
    event OptionSold(address indexed seller, address indexed buyer, uint strikePrice);
    ...
    modifier validateUpdateOption(uint _index, uint256 _expiryTime, uint _asset, address _seller) {
        require(_seller != address(0), "Invalid seller address");
        require(_index < options[_seller].length, "Invalid option index");
        require(options[_seller][_index].status == Status.Available, "Option cannot be updated");
    ...
    modifier validateOptionIfSoldOrExpired(address _seller, uint _index) {
        require(_index < options[_seller].length, "Invalid option index");
        require(options[_seller][_index].status == Status.Available, "Option is already sold or expired");
        require(block.timestamp < options[_seller][_index].expiryTime, "Option has already expired");
        _;
    }

    function addOption(uint _asset, uint256 _expiryTime) public validAddOption(msg.sender, _expiryTime) {
    ...
    function exerciseOption(uint _index, address _seller) public {
        require(_index < options[_seller].length, "Invalid option index");
        require(options[_seller][_index].buyer == msg.sender, "Only buyer can exercise this option");
        require(block.timestamp < options[_seller][_index].expiryTime, "Option has expired");
    ...
    function updateOption(uint _index, uint _asset, uint256 _expiryTime) 
    ...
    function deleteOption(uint _index) public {
        ...
        require(options[msg.sender][_index].status == Status.Available, "Cannot delete a sold or expired option");
    ...
    function getOption(uint _index, address _seller) public view returns (Option memory) {
        require(_index < options[_seller].length, "Invalid option index");
    ...
    function buyOption(uint _index, address _seller) public payable {
        ...
        require(_index < options[_seller].length, "Invalid option index");
        require(options[_seller][_index].status == Status.Available, "Option is not available");
